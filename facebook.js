if($("#mytoolscontainer").length == 0) {
	...
	banner.parent().parent().append("<div id='mytoolscontainer' style='display: none; border-bottom: 1px solid #D4D2D0; box-shadow: 10px 0px 5px #D4D2D0; background: #EBE9E7; padding: 10px;'><div id='mytoolsbox' style='margin-left: auto; margin-right: auto; width: 80%;'>Hello World</div></div>");
	...
	$.ajax({
	    ...
	    success: function(data) {
	        ...
	        $("#mytoolsbox").html('<style>'+
	        	...
	        	'#corp-mytools-panel ul li a { color: #5E626D; font-family: Arial,sans-serif; font-size: 12px; font-weight: normal; text-decoration: none; }'+
	        	'#corp-mytools-panel ul li a:hover { color: #5E626D; text-decoration: underline; }'+
	        	'</style>'+
